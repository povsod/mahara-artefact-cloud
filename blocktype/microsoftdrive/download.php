...
safe_require('blocktype', 'cloud/microsoftdrive');
...
if ($save) {
    ...
    $saveform = pieform(array(
        ...
        'elements'   => array(
            ...
            'submit' => array(
                ...
                'goto' => get_config('wwwroot') . 'artefact/cloud/blocktype/microsoftdrive/manage.php',
            )
        ),
    ));
    ...
    $smarty->display('blocktype:microsoftdrive:save.tpl');
} else {
    // Download file
    $file = PluginBlocktypeMicrosoftdrive::get_file_info($id, $owner);
    $content = PluginBlocktypeMicrosoftdrive::download_file($id, $owner);
...
function saveform_submit(Pieform $form, $values) {
    ...
    $file = PluginBlocktypeMicrosoftdrive::get_file_info($values['fileid']);
    ...
    $content = PluginBlocktypeMicrosoftdrive::download_file($file['id']); 
    ...
    // Redirect
    redirect(get_config('wwwroot') . 'artefact/cloud/blocktype/microsoftdrive/manage.php');
}
