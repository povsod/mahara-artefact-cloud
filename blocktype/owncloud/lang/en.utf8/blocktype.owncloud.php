<?php
/**
 *
 * @package    mahara
 * @subpackage blocktype-owncloud
 * @author     Gregor Anzelj
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL
 * @copyright  (C) 2012-2017 Gregor Anzelj, info@povsod.com
 *
 */

defined('INTERNAL') || die();

$string['title'] = 'ownCloud';
$string['description'] = 'Select files from ownCloud cloud';

$string['service'] = 'ownCloud';
$string['servicename'] = 'ownCloud';

$string['owncloudgeneral'] = 'ownCloud Settings';
$string['servicetitle'] = 'Service title';
$string['servicetitledesc'] = 'Enter the title of your ownCloud service in case you have renamed it, otherwise leave this empty.';
$string['subservice'] = 'Select sub-service';
$string['subservicedesc'] = 'Sub-services contain graphical customizations (e.g. custom icons and images) of ownCloud.';
$string['subservice.arnes'] = 'arnes MAPA';

$string['webdavurl'] = 'WebDAV URL';
$string['webdavurldesc'] = 'Enter WebDAV URL to allow access to your ownCloud service. It should be something like <strong>https://my.owncloud.com/remote.php/webdav/</strong>';
$string['webdavproxy'] = 'WebDAV Proxy';
$string['webdavproxydesc'] = 'Enter WebDAV Proxy to allow access to your ownCloud service. It should be something like <strong>locahost:8888</strong>';

$string['AAIlogin'] = 'If you are accessing ownCloud via AAI login, then you must %sset password%s since WebDAV is not accessible via AAI login. Then enter your username and password in the form below.';

$string['selectfiles'] = 'Select files';
$string['revokeconnection'] = 'Revoke connection to ownCloud';
$string['connecttoowncloud'] = 'Connect to ownCloud';

