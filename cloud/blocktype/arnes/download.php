...
safe_require('blocktype', 'cloud/arnes');
...
if ($save) {
    ...
    $saveform = pieform(array(
        ...
        'elements'   => array(
            ...
            'submit' => array(
                ...
                'goto' => get_config('wwwroot') . 'artefact/cloud/blocktype/arnes/manage.php',
            )
        ),
    ));
    ...
    $smarty->display('blocktype:arnes:save.tpl');
} else {
    // Download file
    $file = PluginBlocktypeArnes::get_file_info($id);
    $content = PluginBlocktypeArnes::download_file($id);
...
function saveform_submit(Pieform $form, $values) {
    ...
    $file = PluginBlocktypeArnes::get_file_info($values['fileid']);
    ...
    $content = PluginBlocktypeArnes::download_file($values['fileid']); 
    ...
    // Redirect
    redirect(get_config('wwwroot') . 'artefact/cloud/blocktype/arnes/manage.php');
}
