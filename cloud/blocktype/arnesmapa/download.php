...
safe_require('blocktype', 'cloud/arnesmapa');
...
if ($save) {
    ...
    $saveform = pieform(array(
        ...
        'elements'   => array(
            ...
            'submit' => array(
                ...
                'goto' => get_config('wwwroot') . 'artefact/cloud/blocktype/arnesmapa/manage.php',
            )
        ),
    ));
    ...
    $smarty->display('blocktype:arnesmapa:save.tpl');
} else {
    // Download file
    $ownerid = null;
    if ($viewid > 0) {
    ...
    $file = PluginBlocktypeArnesmapa::get_file_info($id, $ownerid);
    $content = PluginBlocktypeArnesmapa::download_file($id, $ownerid);
...
function saveform_submit(Pieform $form, $values) {
    ...
    $file = PluginBlocktypeArnesmapa::get_file_info($values['fileid']);
    ...
    $content = PluginBlocktypeArnesmapa::download_file($file['id']); 
    ...
    // Redirect
    redirect(get_config('wwwroot') . 'artefact/cloud/blocktype/arnesmapa/manage.php');
}
