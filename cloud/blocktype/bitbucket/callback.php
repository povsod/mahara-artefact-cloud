...
$token = ArtefactTypeCloud::get_user_preferences('bitbucket', $USER->get('id'));
$token = array_merge($token, array('oauth_verifier' => $oauth_verifier));

if (isset($oauth_token) && $oauth_token == $token['oauth_token']) {
    PluginBlocktypeBitbucket::access_token($token);
    $SESSION->add_ok_msg(get_string('accesstokensaved', 'blocktype.cloud/bitbucket'));
} else {
    $SESSION->add_error_msg(get_string('accesstokensavefailed', 'blocktype.cloud/bitbucket'));
}
