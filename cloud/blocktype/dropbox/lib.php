...
require_once('lib/api.php'); // Dropbox API methods
...
class PluginBlocktypeDropbox extends PluginBlocktypeCloud {
	...
	public function request_token() {
		...
		$consumer = self::consumer_tokens();
		if (!empty($consumer['key']) && !empty($consumer['secret'])) {
			$request = DropboxAPI::get_request_token('POST', $cloud, $consumer);
			if (!empty($request)) {
	...
	public function access_token($params) {
		...
		$consumer = self::consumer_tokens();
		if (!empty($consumer['key']) && !empty($consumer['secret'])) {
			$request = DropboxAPI::get_access_token('POST', $cloud, $consumer, $params);
			if (!empty($request)) {
				list($info, $headers, $body, $body_parsed) = $request;
				if ($info['http_code'] == 200 && !empty($body)) {
	...
	public function account_info() {
		...
		$usertoken = self::user_tokens($USER->get('id'));
		if (!empty($consumer['key']) && !empty($consumer['secret'])) {
			$request = DropboxAPI::make_api_call('POST', '/account/info', $cloud, $consumer, $usertoken, array(), false);
			if (!empty($request)) {
	...
	public function get_filelist($folder_id='/', $selected=array()) {
		...
		$usertoken = self::user_tokens($USER->get('id'));
		$params    = array();
		$options   = array('root' => 'dropbox', 'path' => $folder_id);
		if (!empty($consumer['key']) && !empty($consumer['secret'])) {
			$request = DropboxAPI::make_api_call('POST', '/metadata', $cloud, $consumer, $usertoken, $params, $options, false);
			if (!empty($request)) {
				list($info, $headers, $body) = $request;
				if ($info['http_code'] == 200 && !empty($body)) {
					$data = json_decode($body);
			...
			if (!empty($request)) {
				...
				if ($info['http_code'] == 200 && !empty($body)) {
					...
					$info = array(
						...
						'size'        => $data->bytes, // or maybe $data->size?
	...
	public function get_file_info($file_id='/') {
		global $USER, $THEME;
		//$file_id = rawurlencode(ltrim($file_id, '/'));
	...
	public function download_file($file_id='/') {
		global $USER;
		//$file_id = ltrim(urldecode($file_id), '/');
		...
		$options   = array('root' => 'dropbox', 'path' => $file_id);
		if (!empty($consumer['key']) && !empty($consumer['secret'])) {
			$request = DropboxAPI::make_api_call('GET', '/files', $cloud, $consumer, $usertoken, $params, $options, true);
			return $request;
 		} else {
			throw new ConfigException('Can\'t find Dropbox consumer key and/or consumer secret.');
		}
