{if $microheaders}{include file="viewmicroheader.tpl"}{else}{include file="header.tpl"}{/if}
        ...
        <h4>
            <div class="fl filedata-thumb">
                ...
                {else}
                {if $data.type == 'file'}<a href="{$WWWROOT}artefact/cloud/blocktype/microsoftdrive/download.php?id={$id}&view={$viewid}">{/if}
                <img alt="" src="{theme_url filename="images/file.png"}">
                {if $data.type == 'file'}</a>{/if}
                {/if}
            </div>
            {if $data.type == 'file'}<a href="{$WWWROOT}artefact/cloud/blocktype/microsoftdrive/download.php?id={$id}&view={$viewid}">{$data.name}</a>{else}{$data.name}{/if}
        </h4>
        ...
        <table class="filedata">
            <tbody>
                ...
                <tr>
                    ...
                    <a href="{$WWWROOT}artefact/cloud/blocktype/microsoftdrive/download.php?id={$id}&view={$viewid}">{str tag=Download section=artefact.file}</a>
                    </td>
                </tr>
                {/if}
            </tbody>
