...
safe_require('blocktype', 'cloud/skydrive');
    ...
    $saveform = pieform(array(
        ...
        'elements'   => array(
            ...
            'submit' => array(
                ...
                'goto' => get_config('wwwroot') . 'artefact/cloud/blocktype/skydrive/manage.php',
            )
        ),
    ));
    
    $smarty = smarty();
    //$smarty->assign('SERVICE', 'skydrive');
    $smarty->assign('PAGEHEADING', get_string('savetomahara', 'artefact.cloud'));
    $smarty->assign('saveform', $saveform);
    $smarty->display('blocktype:skydrive:save.tpl');
} else {
    // Download file
    $file = PluginBlocktypeSkydrive::get_file_info($id);
    $content = PluginBlocktypeSkydrive::download_file($id);
...
function saveform_submit(Pieform $form, $values) {
    ...
    $file = PluginBlocktypeSkydrive::get_file_info($values['fileid']);
    ...
    $content = PluginBlocktypeSkydrive::download_file($file['id']); 
    ...
    // Redirect
    redirect(get_config('wwwroot') . 'artefact/cloud/blocktype/skydrive/manage.php');
}
