...
safe_require('blocktype', 'cloud/soundcloud');
    ...
    $saveform = pieform(array(
        ...
        'elements'   => array(
            ...
            'submit' => array(
                ...
                'goto' => get_config('wwwroot') . 'artefact/cloud/blocktype/soundcloud/manage.php',
            )
        ),
    ));
    
    $smarty = smarty();
    //$smarty->assign('SERVICE', 'soundcloud');
    $smarty->assign('PAGEHEADING', get_string('savetomahara', 'artefact.cloud'));
    $smarty->assign('saveform', $saveform);
    $smarty->display('blocktype:soundcloud:save.tpl');
} else {
    // Download file
    $file = PluginBlocktypeSoundcloud::get_file_info($id);
    $content = PluginBlocktypeSoundcloud::download_file($id);
...
function saveform_submit(Pieform $form, $values) {
    ...
    $file = PluginBlocktypeSoundcloud::get_file_info($values['fileid']);
    ...
    $content = PluginBlocktypeSoundcloud::download_file($file['id']); 
    ...
    // Redirect
    redirect(get_config('wwwroot') . 'artefact/cloud/blocktype/soundcloud/manage.php');
}
