...
safe_require('blocktype', 'cloud/sugarsync');
...
if ($save) {
    ...
    $saveform = pieform(array(
        ...
        'elements'   => array(
            ...
            'submit' => array(
                ...
                'goto' => get_config('wwwroot') . 'artefact/cloud/blocktype/sugarsync/manage.php',
            )
        ),
    ));
    ...
    $smarty->display('blocktype:sugarsync:save.tpl');
} else {
    // Download file
    $ownerid = null;
    if ($viewid > 0) {
    ...
    $file = PluginBlocktypeSugarsync::get_file_info($id, $ownerid);
    $content = PluginBlocktypeSugarsync::download_file($id, $ownerid);
...
function saveform_submit(Pieform $form, $values) {
    ...
    $file = PluginBlocktypeSugarsync::get_file_info($values['fileid']);
    ...
    $content = PluginBlocktypeSugarsync::download_file($file['id']); 
    ...
    // Redirect
    redirect(get_config('wwwroot') . 'artefact/cloud/blocktype/sugarsync/manage.php');
}
