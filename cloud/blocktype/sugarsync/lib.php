...
require_once('lib/api.php'); // SugarSync API methods
...
class PluginBlocktypeSugarsync extends PluginBlocktypeCloud {
	...
	public function cloud_info() {
		return array(
			...
			//'contenturl' => 'https://api.sugarsync.com/',
			'authurl'     => 'https://api.sugarsync.com/app-authorization/',
		);
	}
	...
	public function access_token($refresh_token) {
		...
		$token  = $cloud['authurl'] . $refresh_token;
...
XML;
		...
		if (!empty($consumer['key']) && !empty($consumer['secret'])) {
			$request = SugarSyncAPI::get_access_token('POST', $cloud, $consumer, $request_body);
			if (!empty($request)) {
				list($info, $headers, $body, $access_token) = $request;
				if ($info['http_code'] == 201 /* HTTP/1.1 201 Created */ && !empty($body)) {
	...
	public function account_info() {
		...
		if (!empty($consumer['key']) && !empty($consumer['secret'])) {
			$request = SugarSyncAPI::make_api_call('GET', $accesstoken['user'], $cloud, $consumer, $accesstoken, $accesstoken['userid']);
			if (!empty($request)) {
	...
	public function get_filelist($folder_id=0, $selected=array()) {
		...
		if (!empty($consumer['key']) && !empty($consumer['secret'])) {
			if (strlen($folder_id) > 1) {
				$request = SugarSyncAPI::make_api_call('GET', $cloud['baseurl'].'folder/'.$folder_id.'/contents', $cloud, $consumer, $accesstoken, $accesstoken['userid']);
			} else {
				$request = SugarSyncAPI::make_api_call('GET', $accesstoken['user'].'/folders/contents', $cloud, $consumer, $accesstoken, $accesstoken['userid']);
			}
			if (!empty($request)) {
				list($info, $headers, $body) = $request;
				if ($info['http_code'] == 200 && !empty($body)) {
	 ...
	 * and the checsugarsync or radio form elements are added, to allow user to select
	...
	public function get_folder_info($folder_id=0) {
		...
		$accesstoken = self::access_token($usertoken['refresh_token']);
		if (!empty($consumer['key']) && !empty($consumer['secret'])) {
			$request = SugarSyncAPI::make_api_call('GET', $cloud['baseurl'].'folder/'.$folder_id, $cloud, $consumer, $accesstoken, $accesstoken['userid']);
			if (!empty($request)) {
				list($info, $headers, $body) = $request;
				if ($info['http_code'] == 200 && !empty($body)) {
					$data = oauth_parse_xml($body);
	...
	public function get_file_info($file_id=0) {
		...
		$accesstoken = self::access_token($usertoken['refresh_token']);
		if (!empty($consumer['key']) && !empty($consumer['secret'])) {
			$request = SugarSyncAPI::make_api_call('GET', $cloud['baseurl'].'file/'.$file_id, $cloud, $consumer, $accesstoken, $accesstoken['userid']);
			if (!empty($request)) {
				list($info, $headers, $body) = $request;
				if ($info['http_code'] == 200 && !empty($body)) {
					$data = oauth_parse_xml($body);
	...
	public function download_file($file_id=0) {
		...
		if (!empty($consumer['key']) && !empty($consumer['secret'])) {
			...
			$result = '';
			$headers[] = 'Authorization: https://api.sugarsync.com/authorization/' . $accesstoken['accesstoken'];
